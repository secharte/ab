...
public class TransitionSystem {
    ...
    private void applySelEv() throws JasonException {
        ...
        if (conf.C.hasEvent()) {
            ...
            if (confP.C.SE != null) {
               //Modificacion para obligar a ejecutar la selectora option
            	/*
    ...
    public List<Option> applicablePlans(List<Option> rp) throws JasonException {
        ...
        if (rp != null) {
            ...
            for (Option opt: rp) {
                ...
                //Modificacion para chanceOfSuccess o degOfCert
                boolean ok = false;
                String scont =context.toString();
                if(scont.indexOf("chanceOfSucess")!= -1){
                	Double g = -1.0;
                	int i    = scont.indexOf("chanceOfSucess");
            		//encontro la parte del termino que posee "degOfCert"
            		if(i!=-1){
            			if(scont.charAt(i+15) != '1'){
	            			String sub = scont.substring(i+15,i+18);
	            			g = new Double(sub);
	            		}else {
	            			g = new Double(1.0);
	            		}
                	...
                	//System.out.println("Contexto con grado: " + context + "..."+ g);
                	if(g > c){
                		
                		ok = true;
                	}
                }else if(scont.indexOf("degOfCert")!= -1){
                	Double g = -1.0;
                	int i    = scont.indexOf("degOfCert");
            		//encontro la parte del termino que posee "degOfCert"
            		if(i!=-1){
            			if(scont.charAt(i+10) != '1'){
	            			String sub = scont.substring(i+10,i+13);
	            			g = new Double(sub);
	            		}else {
	            			g = new Double(1.0);
	            		}
                	...
                	if(g > c){
                	 ...
                	 //AB:que no haya anotaciones en el contexto
                	 ok=true;
                 }
                if(ok == true){
	                if (context == null) { // context is true
	                    ...
	                    /* AB:Tenemos el problema que el contexto posee por ejemplo viento_leve[chanceOfSucess(0.5)]
	                     * y no unifica con lo que esta en la BB por ejemplo viento_leve[degOfCert(0.8)].
	                     */
